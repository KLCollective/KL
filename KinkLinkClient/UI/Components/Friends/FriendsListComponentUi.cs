...
public class PairsListComponentUi(
    FriendsListComponentUiController controller,
    ...
    public void Draw(bool displayAddFriendsBox = false, bool displayOfflineFriends = false)
    {
        ...
        if (
            ImGui.BeginChild(
                ...
                new Vector2(KinkLinkDimensions.NavBar.X - KinkLinkImGui.WindowPadding.X, 0),
                false,
        ...
        var width =
            ImGui.GetWindowWidth() - ImGui.GetCursorPosX() - KinkLinkImGui.WindowPadding.X * 2;
        ...
        SharedUserInterfaces.ContentBox(
            ...
            () =>
            {
                ...
                ImGui.SetNextItemWidth(
                    width - KinkLinkImGui.WindowPadding.X - KinkLinkDimensions.IconButton.X
                );
                if (
                    ImGui.InputTextWithHint(
                        "###SearchFriendInputText",
                        "Friend",
                ...
                if (controller.Filter.SortMode is FilterSortMode.Alphabetically)
                {
                    if (
                        SharedUserInterfaces.IconButton(
                            FontAwesomeIcon.SortAlphaDown,
                            KinkLinkDimensions.IconButton,
                            "Filtering by Alphabetical"
                        )
                    )
                ...
                else
                {
                    if (
                        SharedUserInterfaces.IconButton(
                            ...
                            KinkLinkDimensions.IconButton,
                            "Filtering by most recent interaction"
                        )
                    )
                        controller.ToggleSortMode();
                }
        ...
        var height = displayAddFriendsBox
            ? KinkLinkImGui.WindowPadding.Y * 3
                + KinkLinkImGui.FramePadding.Y * 4
                + ImGui.GetFontSize() * 2
                + KinkLinkImGui.ItemSpacing.Y * 2
            : 0;
        ...
        if (ImGui.BeginChild("###PermissionViewFriendsList", new Vector2(0, -height), true))
        {
            ...
            if (displayOfflineFriends && pending.Count > 0)
            {
                ...
                foreach (var friend in pending)
                {
                    if (
                        ImGui.Selectable(
                            $"{friend.NoteOrFriendCode}##{friend.FriendCode}",
                            selectionManager.Contains(friend)
                        )
                    )
            ...
            SharedUserInterfaces.ContentBox(
                ...
                () =>
                {
                    ...
                    ImGui.InputTextWithHint(
                        "###AddFriendInputText",
                        "Friend code",
                        ref controller.FriendCodeToAdd,
                        32
                    );
