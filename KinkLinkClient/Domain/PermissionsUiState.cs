...
public class PermissionsUiState
{
    ...
    public bool Equals(PermissionsUiState other)
    {
        ...
        // Gag permissions
        if (CanApplyGag != other.CanApplyGag)
            return false;
        if (CanLockGag != other.CanLockGag)
            return false;
        if (CanUnlockGag != other.CanUnlockGag)
            return false;
        if (CanRemoveGag != other.CanRemoveGag)
            return false;
        if (CanForceEnableGagGlamour != other.CanForceEnableGagGlamour)
            return false;
        if (CanEnableGarbler != other.CanEnableGarbler)
            return false;
        if (CanLockGarbler != other.CanLockGarbler)
            return false;
        if (CanSetGarblerChannels != other.CanSetGarblerChannels)
            return false;
        if (CanLockGarblerChannels != other.CanLockGarblerChannels)
            return false;

        // Wardrobe permissions
        if (CanApplyWardrobe != other.CanApplyWardrobe)
            return false;
        if (CanLockWardrobe != other.CanLockWardrobe)
            return false;
        if (CanUnlockWardrobe != other.CanUnlockWardrobe)
            return false;
        if (CanRemoveWardrobe != other.CanRemoveWardrobe)
            return false;
        if (CanForceEnableWardrobeGlamour != other.CanForceEnableWardrobeGlamour)
            return false;

        // Moodle permissions
        if (CanApplyOwn != other.CanApplyOwn)
            return false;
        if (CanApplyPairs != other.CanApplyPairs)
            return false;
        if (CanLockMoodles != other.CanLockMoodles)
            return false;
        if (CanUnlockMoodles != other.CanUnlockMoodles)
            return false;
        if (CanRemoveMoodles != other.CanRemoveMoodles)
            return false;
    ...
    public static UserPermissions To(PermissionsUiState permissions)
    {
        ...
        // Gag permissions
        if (permissions.CanApplyGag)
            perms |= InteractionPerms.CanApplyGag;
        if (permissions.CanLockGag)
            perms |= InteractionPerms.CanLockGag;
        if (permissions.CanUnlockGag)
            perms |= InteractionPerms.CanUnlockGag;
        if (permissions.CanRemoveGag)
            perms |= InteractionPerms.CanRemoveGag;
        if (permissions.CanForceEnableGagGlamour)
            perms |= InteractionPerms.CanForceEnableGlamourGag;
        if (permissions.CanEnableGarbler)
            perms |= InteractionPerms.CanEnableGarbler;
        if (permissions.CanLockGarbler)
            perms |= InteractionPerms.CanLockGarbler;
        if (permissions.CanSetGarblerChannels)
            perms |= InteractionPerms.CanSetGarblerChannels;
        if (permissions.CanLockGarblerChannels)
            perms |= InteractionPerms.CanLockGarblerChannels;

        // Wardrobe permissions
        if (permissions.CanApplyWardrobe)
            perms |= InteractionPerms.CanApplyWardrobe;
        if (permissions.CanLockWardrobe)
            perms |= InteractionPerms.CanLockWardrobe;
        if (permissions.CanUnlockWardrobe)
            perms |= InteractionPerms.CanUnlockWardrobe;
        if (permissions.CanRemoveWardrobe)
            perms |= InteractionPerms.CanRemoveWardrobe;
        if (permissions.CanForceEnableWardrobeGlamour)
            perms |= InteractionPerms.CanForceEnableGlamour;

        // Moodle permissions
        if (permissions.CanApplyOwn)
            perms |= InteractionPerms.CanApplyOwnMoodles;
        if (permissions.CanApplyPairs)
            perms |= InteractionPerms.CanApplyPairsMoodles;
        if (permissions.CanLockMoodles)
            perms |= InteractionPerms.CanLockMoodles;
        if (permissions.CanUnlockMoodles)
            perms |= InteractionPerms.CanUnlockMoodles;
        if (permissions.CanRemoveMoodles)
            perms |= InteractionPerms.CanRemoveMoodles;
