...
{
    public bool GarblerEnabled,
        GarblerLocked,
        GarblerChannelsLocked;
    ...
    {
        if (GarblerEnabled != other.GarblerEnabled)
            return false;
        if (GarblerLocked != other.GarblerLocked)
            return false;
        if (GarblerChannelsLocked != other.GarblerChannelsLocked)
            return false;
        ...
        if (Alliance != other.Alliance)
            return false;
        if (FreeCompany != other.FreeCompany)
            return false;
        if (PvPTeam != other.PvPTeam)
            return false;
        if (Echo != other.Echo)
            return false;
        if (Roleplay != other.Roleplay)
            return false;
    ...
    public static UserGarblerSettings To(GarblerSettingsUiState garblerState)
    {
        ...
        // Speak Permissions
        if (garblerState.Say)
            chatchannels |= GarblerChannels.Say;
        if (garblerState.Yell)
            chatchannels |= GarblerChannels.Yell;
        if (garblerState.Shout)
            chatchannels |= GarblerChannels.Shout;
        if (garblerState.Tell)
            chatchannels |= GarblerChannels.Tell;
        if (garblerState.Party)
            chatchannels |= GarblerChannels.Party;
        if (garblerState.Alliance)
            chatchannels |= GarblerChannels.Alliance;
        if (garblerState.FreeCompany)
            chatchannels |= GarblerChannels.FreeCompany;
        if (garblerState.PvPTeam)
            chatchannels |= GarblerChannels.PvPTeam;
        if (garblerState.Echo)
            chatchannels |= GarblerChannels.Echo;
        if (garblerState.Roleplay)
            chatchannels |= GarblerChannels.Roleplay;

        // Linkshell Permissions
        if (garblerState.Ls1)
            chatchannels |= GarblerChannels.Ls1;
        if (garblerState.Ls2)
            chatchannels |= GarblerChannels.Ls2;
        if (garblerState.Ls3)
            chatchannels |= GarblerChannels.Ls3;
        if (garblerState.Ls4)
            chatchannels |= GarblerChannels.Ls4;
        if (garblerState.Ls5)
            chatchannels |= GarblerChannels.Ls5;
        if (garblerState.Ls6)
            chatchannels |= GarblerChannels.Ls6;
        if (garblerState.Ls7)
            chatchannels |= GarblerChannels.Ls7;
        if (garblerState.Ls8)
            chatchannels |= GarblerChannels.Ls8;
        if (garblerState.Cwl1)
            chatchannels |= GarblerChannels.Cwl1;
        if (garblerState.Cwl2)
            chatchannels |= GarblerChannels.Cwl2;
        if (garblerState.Cwl3)
            chatchannels |= GarblerChannels.Cwl3;
        if (garblerState.Cwl4)
            chatchannels |= GarblerChannels.Cwl4;
        if (garblerState.Cwl5)
            chatchannels |= GarblerChannels.Cwl5;
        if (garblerState.Cwl6)
            chatchannels |= GarblerChannels.Cwl6;
        if (garblerState.Cwl7)
            chatchannels |= GarblerChannels.Cwl7;
        if (garblerState.Cwl8)
            chatchannels |= GarblerChannels.Cwl8;

        return new UserGarblerSettings(
            garblerState.GarblerEnabled,
            garblerState.GarblerLocked,
            garblerState.GarblerChannelsLocked,
            chatchannels
        );
    }
}
