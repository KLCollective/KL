...
public class DiscordInteractionHandler
{
    ...
    private readonly ulong _alphaTesterRoleId;
    ...
    )
    {
        ...
        _alphaTesterRoleId = config.Bot.AlphaTesterRoleId;
    ...
    public async Task EnsureRegistrationPromptExistsAsync()
    {
        try
        {
            ...
            {
                _logger.LogError(
                    "Channel {ChannelId} not found in guild {GuildId} during startup",
                    _channelId,
                    _guildId
                );
                return;
            ...
            if (!hasRegistrationPrompt)
            {
                _logger.LogInformation(
                    "No registration prompt found in channel {ChannelId}, creating new one",
                    channel.Id
                );
                await CreatePrompt();
            }
            ...
            {
                _logger.LogInformation(
                    "Registration prompt already exists in channel {ChannelId}",
                    channel.Id
                );
            }
        }
    ...
    private bool IsRegistrationPromptMessage(IMessage message)
    {
        return message.Embeds?.Any(embed =>
                embed.Title?.Contains(
                    "kinklink account management",
                    StringComparison.OrdinalIgnoreCase
                ) == true
            ) == true;
    }

    private bool IsAlphaTester(SocketGuildUser user)
    {
        return _alphaTesterRoleId != 0 && user.Roles.Any(r => r.Id == _alphaTesterRoleId);
    }
    ...
    public async Task<RegistrationResponse> CreatePrompt()
    {
        try
        {
            ...
            var embed = new EmbedBuilder
            {
                ...
                Description =
                    "Welcome to KinkLink! To get started, please click the button below to create your account",
                Color = Color.Blue,
            };
            ...
            var buttonBuilder = new ComponentBuilder().WithButton(
            ...
            try
            {
                ...
                _logger.LogInformation(
                    "Registration prompt sent and pinned to channel {ChannelId} in guild {GuildId}",
                    channel.Id,
                    guild.Id
                );
            }
            catch (Exception pinEx)
            {
                _logger.LogWarning(
                    pinEx,
                    "Failed to pin registration message to channel {ChannelId} in guild {GuildId}",
                    channel.Id,
                    guild.Id
                );
            }
    ...
    private async Task HandleRegistrationStartAsync(SocketMessageComponent component)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling registration start for user {UserId}",
                component.User.Id
            );
            await component.RespondAsync(
    ...
    private async Task HandleNewUserConfirmation(
    ...
    )
    {
        ...
        var buttonBuilder = new ComponentBuilder().WithButton(
            "I understand, I wish to create my account",
            "register_confirmed",
            ButtonStyle.Primary
        );
    ...
    private async Task HandleNewUserRegistration(
        SocketMessageComponent component,
        SocketGuildUser user
    )
    {
        ...
        if (response.Success)
        {
            ...
            embed.AddField(
                "⚠️ Important",
                "• Your Profile has a UID is public and used to connect with others\n"
            ...
            var buttonBuilder = new ComponentBuilder()
                .WithButton("🎭 Create First Profile", "create_first_profile", ButtonStyle.Primary)
            ...
            _logger.LogInformation(
                "User {Username} ({UserId}) successfully registered Discord account",
                user.Username,
                user.Id
            );
        }
        else
        {
            ...
            _logger.LogWarning(
                "Registration failed for user {Username} ({UserId}): {ErrorMessage}",
                user.Username,
                user.Id,
    ...
    private async Task ShowAccountManagementAsync(
        SocketMessageComponent component,
        SocketGuildUser user,
        KinkLinkCommon.Database.User userRecord
    )
    {
        try
        {
            ...
            var embed = new EmbedBuilder
            {
                ...
                Description =
                    $"Welcome {user.Username}! Manage your KinkLink account settings and profiles.",
                Color = Color.Blue,
            };
            ...
            if (profiles.Any())
            {
                var profileList = string.Join(
                    "\n",
                    profiles.Select(p =>
                        $"• **{p.Uid}**{(string.IsNullOrEmpty(p.Alias) ? "" : $" - {p.Alias}")}"
                    )
                );
    ...
    private async Task HandleAccountRemovalAsync(SocketMessageComponent component)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling account removal prompt for user {UserId}",
                component.User.Id
            );
            await component.RespondAsync(
                "An unexpected error occurred. Please try again later.",
    ...
    private async Task HandleSkipUIDCreationAsync(SocketMessageComponent component)
    {
        try
        {
            var embed = new EmbedBuilder
            {
                ...
                Description =
                    "You can create your UID later through the bot commands or by registering again.",
                Color = Color.Green,
            };
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling skip UID creation for user {UserId}",
                component.User.Id
            );
            await component.RespondAsync(
                "An unexpected error occurred. Please try again later.",
    ...
    private async Task HandleModalSubmissionAsync(SocketModal modal)
    {
        ...
        switch (modal.Data.CustomId)
        {
            ...
            case "regenerate_secret_modal":
                ...
                break;
            case "create_profile_custom_modal":
                await HandleCreateProfileCustomModalAsync(modal);
                break;
    ...
    private async Task HandleAccountRemovalConfirmationAsync(SocketModal modal)
    {
        try
        {
            var confirmation = modal
                .Data.Components.FirstOrDefault(x => x.CustomId == "confirmation")
                ?.Value;

            if (confirmation?.Trim().ToUpper() != "DELETE")
            {
                await modal.RespondAsync(
                    "Confirmation text incorrect. Account deletion cancelled.",
                    ephemeral: true
                );
                return;
            }
            ...
            if (response.Success)
            {
                var embed = new EmbedBuilder
                {
                    ...
                    Description =
                        "Your KinkLink account and all associated data have been permanently deleted.",
                    Color = Color.Red,
                };

                await modal.RespondAsync(embed: embed.Build(), ephemeral: true);
                _logger.LogInformation(
                    "User {Username} ({UserId}) successfully deleted their account",
                    modal.User.Username,
                    modal.User.Id
                );
            }
            else
            {
                ...
                _logger.LogWarning(
                    "Account deletion failed for user {Username} ({UserId}): {ErrorMessage}",
                    modal.User.Username,
                    modal.User.Id,
                    response.ErrorMessage
                );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling account removal confirmation for user {UserId}",
                modal.User.Id
            );
            await modal.RespondAsync(
    ...
    private async Task HandleUIDRemovalConfirmationAsync(SocketModal modal)
    {
        try
        {
            ...
            {
                await modal.RespondAsync(
                    "Confirmation text incorrect. UID deletion cancelled.",
                    ephemeral: true
                );
                return;
            }
            ...
            if (response.Success)
            {
                ...
                await modal.RespondAsync(embed: embed.Build(), ephemeral: true);
                _logger.LogInformation(
                    "User {Username} ({UserId}) successfully deleted UID {Uid}",
                    modal.User.Username,
                    modal.User.Id,
                    uid
                );
            ...
            else
            {
                ...
                await modal.RespondAsync(embed: errorEmbed.Build(), ephemeral: true);
                _logger.LogWarning(
                    "UID deletion failed for user {Username} ({UserId}), UID {Uid}: {ErrorMessage}",
                    modal.User.Username,
                    modal.User.Id,
                    uid,
                    response.ErrorMessage
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling UID removal confirmation for user {UserId}",
                modal.User.Id
            );
            await modal.RespondAsync(
                "An unexpected error occurred while deleting your UID. Please try again later.",
                ephemeral: true
            );
        }
    }
    ...
    private async Task HandleCreateProfileAsync(SocketMessageComponent component)
    {
        ...
        {
            var user = component.User as SocketGuildUser;
            var isAlpha = user != null && IsAlphaTester(user);

            var modal = new ModalBuilder()
                .WithTitle("Create Profile")
                .WithCustomId(isAlpha ? "create_profile_custom_modal" : "create_profile_modal")
                .AddTextInput(
                    "What would you like your profile alias to be?",
                    "alias",
                    TextInputStyle.Short,
                    placeholder: "Optional - leave blank for no alias",
                    required: false,
                    maxLength: 20
                );
            ...
            {
                modal.AddTextInput(
                    "Preferred Last 4 Digits (Alpha Testers only)",
                    "last_four",
                    TextInputStyle.Short,
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling create profile prompt for user {UserId}",
                component.User.Id
            );
            await component.RespondAsync(
                "An unexpected error occurred. Please try again later.",
    ...
    private async Task HandleCreateProfileCustomModalAsync(SocketModal modal)
    {
        try
        {
            var alias = modal
                .Data.Components.FirstOrDefault(x => x.CustomId == "alias")
                ?.Value?.Trim();
            var lastFour = modal
                .Data.Components.FirstOrDefault(x => x.CustomId == "last_four")
                ?.Value?.Trim();

            string? customSuffix = null;
            if (!string.IsNullOrEmpty(lastFour) && lastFour.Length == 4)
            {
                if (!lastFour.All(char.IsLetterOrDigit))
                {
                    await modal.RespondAsync(
                        "Custom suffix must be alphanumeric (A-Z, 0-9 only).",
                        ephemeral: true
                    );
                    return;
                }
                customSuffix = lastFour.ToUpperInvariant();
            }

            if (!string.IsNullOrEmpty(alias) && alias.Length > 20)
            {
                await modal.RespondAsync(
                    "Alias is too long. Maximum 20 characters allowed.",
                    ephemeral: true
                );
                return;
            }

            var response = await _registrationService.CreateProfileWithAliasAsync(
                modal.User.Id,
                alias,
                customSuffix
            );
            ...
            if (response.Success)
            {
                ...
                embed.AddField(
                    "Next Steps",
                    "You can now use this UID in the game client to connect with others."
                );
                ...
                _logger.LogInformation(
                    "User {Username} ({UserId}) created new profile {Uid}",
                    modal.User.Username,
                    modal.User.Id,
                    response.UID
                );
            ...
            else
            {
                ...
                await modal.RespondAsync(embed: errorEmbed.Build(), ephemeral: true);
                _logger.LogWarning(
                    "Profile creation failed for user {Username} ({UserId}): {ErrorMessage}",
                    modal.User.Username,
                    modal.User.Id,
                    response.ErrorMessage
                );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling create profile modal for user {UserId}",
                modal.User.Id
            );
            await modal.RespondAsync(
    ...
    private async Task HandleDeleteProfilePromptAsync(SocketMessageComponent component)
    {
        try
        {
            ...
            var modal = new ModalBuilder()
                ...
                .AddTextInput(
                    ...
                    "profile_uid",
                    TextInputStyle.Short,
                    placeholder: $"Available profiles: {string.Join(", ", profiles.Select(p => p.Uid))}",
                    required: true
                )
                .AddTextInput(
                    "Type 'DELETE' to confirm",
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling delete profile prompt for user {UserId}",
                component.User.Id
            );
            await component.RespondAsync(
                "An unexpected error occurred. Please try again later.",
    ...
    private async Task HandleRegenerateSecretPromptAsync(SocketMessageComponent component)
    {
        try
        {
            var modal = new ModalBuilder()
                ...
                .AddTextInput(
                    "Type 'CONFIRM' to regenerate your secret",
                    "confirmation",
                    TextInputStyle.Short,
                    placeholder: "Warning: This will invalidate all active sessions!",
                    required: true
                );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling regenerate secret prompt for user {UserId}",
                component.User.Id
            );
            await component.RespondAsync(
                "An unexpected error occurred. Please try again later.",
    ...
    private async Task HandleDeleteAccountPromptAsync(SocketMessageComponent component)
    {
        try
        {
            var modal = new ModalBuilder()
                ...
                .AddTextInput(
                    ...
                    "confirmation",
                    TextInputStyle.Short,
                    placeholder: "This action is irreversible and will delete all your data!",
                    required: true
                );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling delete account prompt for user {UserId}",
                component.User.Id
            );
            await component.RespondAsync(
                "An unexpected error occurred. Please try again later.",
    ...
    private async Task HandleDeleteProfileModalAsync(SocketModal modal)
    {
        try
        {
            var profileUid = modal
                .Data.Components.FirstOrDefault(x => x.CustomId == "profile_uid")
                ?.Value?.Trim();
            var confirmation = modal
                .Data.Components.FirstOrDefault(x => x.CustomId == "confirmation")
                ?.Value?.Trim()
            ...
            {
                await modal.RespondAsync(
                    "Confirmation text incorrect. Profile deletion cancelled.",
                    ephemeral: true
                );
                return;
            }
            ...
            {
                await modal.RespondAsync(
                    "Invalid UID format. UIDs should be 10 alphanumeric characters.",
                    ephemeral: true
                );
                return;
            }
            ...
            if (response.Success)
            {
                ...
                _logger.LogInformation(
                    "User {Username} ({UserId}) deleted profile {ProfileUid}",
                    modal.User.Username,
                    modal.User.Id,
                    profileUid
                );
            ...
            else
            {
                ...
                await modal.RespondAsync(embed: errorEmbed.Build(), ephemeral: true);
                _logger.LogWarning(
                    "Profile deletion failed for user {Username} ({UserId}), UID {ProfileUid}: {ErrorMessage}",
                    modal.User.Username,
                    modal.User.Id,
                    profileUid,
                    response.ErrorMessage
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling delete profile modal for user {UserId}",
                modal.User.Id
            );
            await modal.RespondAsync(
                "An unexpected error occurred while deleting your profile. Please try again later.",
                ephemeral: true
            );
        }
    }
    ...
    private async Task HandleRegenerateSecretModalAsync(SocketModal modal)
    {
        try
        {
            ...
            {
                await modal.RespondAsync(
                    "Confirmation text incorrect. Secret regeneration cancelled.",
                    ephemeral: true
                );
                return;
            }
            ...
            if (response.Success)
            {
                ...
                _logger.LogInformation(
                    "User {Username} ({UserId}) regenerated their secret",
                    modal.User.Username,
                    modal.User.Id
                );
            }
            else
            {
                ...
                await modal.RespondAsync(embed: errorEmbed.Build(), ephemeral: true);
                _logger.LogWarning(
                    "Secret regeneration failed for user {Username} ({UserId}): {ErrorMessage}",
                    modal.User.Username,
                    modal.User.Id,
                    response.ErrorMessage
                );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling regenerate secret modal for user {UserId}",
                modal.User.Id
            );
            await modal.RespondAsync(
                "An unexpected error occurred while regenerating your secret. Please try again later.",
                ephemeral: true
            );
        }
    }
    ...
    private async Task HandleDeleteAccountModalAsync(SocketModal modal)
    {
        try
        {
            ...
            if (response.Success)
            {
                ...
                await modal.RespondAsync(embed: embed.Build(), ephemeral: true);
                _logger.LogInformation(
                    "User {Username} ({UserId}) deleted their account",
                    modal.User.Username,
                    modal.User.Id
                );
            }
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error handling delete account modal for user {UserId}",
                modal.User.Id
            );
            await modal.RespondAsync(
                "An unexpected error occurred while deleting your account. Please try again later.",
